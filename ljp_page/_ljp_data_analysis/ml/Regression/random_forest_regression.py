...
class RandomForestRegressionModel(BaseModel):
    ...
    def preprocess(self, scale: bool = False) -> "RandomForestRegressionModel":
        """
        特征预处理。
    ...
    def fit(self, scale: bool = False, **kwargs) -> "RandomForestRegressionModel":
        """训练随机森林回归。"""
...
def random_forest_regression_auto(
    X: Union[np.ndarray, pd.DataFrame],
    y: Union[np.ndarray, pd.Series],
    scale: bool = False,
    **kwargs,
) -> tuple[RandomForestRegressionModel, np.ndarray]:
    """自动训练随机森林回归并返回训练集预测值。"""
    model = RandomForestRegressionModel(X, y)
    model.fit(scale=scale, **kwargs)
    predictions = model.predict(X)
    return model, predictions


__all__ = ["RandomForestRegressionModel", "random_forest_regression_auto"]
