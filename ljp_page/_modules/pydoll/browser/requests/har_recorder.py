"""HAR 网络记录器，用于捕获和重放浏览器网络流量。

该模块提供内部录音引擎（HarRecorder）和
面向用户的记录对象 (HarCapture)，共同启用 HAR 1.2
从浏览器会话中捕获和导出。"""
...
from ljp_page._modules.pydoll.commands.network_commands import NetworkCommands
...
from ljp_page._modules.pydoll.protocol.network.har_types import (
    Har,
    HarContent,
    HarCookie,
    HarCreator,
...
if TYPE_CHECKING:
    ...
    from ljp_page._modules.pydoll.protocol.network.types import ResourceTiming
    from ljp_page._modules.pydoll.protocol.network.types import Response as CDPResponse
...
class HarRecorder:
    """监听 CDP 网络事件并构建 HAR 条目的内部引擎。

    此类注册 7 个 CDP 网络事件的回调，并将它们关联起来
    通过 requestId，并构建 HAR 1.2 条目。它不适合直接
    使用 — 相反，使用包装此引擎的“tab.request.record()”。"""
    ...
    def _on_data_received(self, event: DataReceivedEvent) -> None:
        ...
        累积每个 requestId 的正文块字节以获得准确的 bodySize。"""
        params = event['params']
        request_id = params['requestId']
    ...
    async def _fetch_response_body(self, request_id: str) -> tuple[str, bool]:
        ...
        返回：
            (body_text, is_base64_encoded) 的元组。失败时返回 ('', False)。"""
        try:
    ...
    def _build_entry(self, pending: dict[str, Any]) -> HarEntry:
        ...
        #不带 ssl 的总和 — connect 已根据 HAR 1.2 规范包含它
        _phases = (
            har_timings['blocked'],
            har_timings['dns'],
            har_timings['connect'],
        ...
        for key, field in [
            ...
            if pending.get(key, ''):
                entry[field] = pending[key]  #类型：忽略[文字必需]
    ...
    def _build_har_response(
        ...
        #来自 dataReceived 块的 bodySize（实际主体字节，无标头开销）
        #对于 304（缓存命中），根据 HAR 规范，bodySize 必须为 0
        #当 body_bytes 为 0 但内容存在时（例如 file:// 协议），
        #回退到 content_size 以与 content.size/text 保持一致。
    ...
    def _build_har_timings(
        ...
        """将 CDP ResourceTiming 转换为 HAR 计时（以毫秒为单位）。

        参数：
            计时：来自responseReceived 的CDP ResourceTiming。
            receive_ms：根据单调时间戳计算的接收时间
                （加载完成.时间戳 - 响应接收.时间戳）。
                如果提供，将覆盖基于标头的计算。"""
    ...
    def _normalize_http_version(protocol: str) -> str:
        """将 CDP 协议字符串规范为 HAR httpVersion 格式。

        CDP 报告“h2”、“h3”、“http/1.0”、“http/1.1”等协议
        或非 HTTP 字符串，如“file”。 HAR 观众期待大写
        HTTP 版本（例如“HTTP/1.1”、“h2”、“h3”）。"""
        if not protocol:
            return ''
    ...
    def _wall_time_to_iso(wall_time: float) -> str:
        """将 CDP wallTime（自纪元以来的秒数）转换为 ISO 8601 字符串。"""
        if not wall_time:
...
class HarCapture:
    """由“tab.request.record()”上下文管理器返回的面向用户的对象。

    提供对记录的 HAR 条目和导出方法的访问
    录制为 HAR 1.2 文件。"""
    ...
    def to_dict(self) -> Har:
        """根据记录的条目构建完整的 HAR 1.2 字典。

        返回：
            准备好进行 JSON 序列化的完整 HAR 1.2 字典。"""
        return Har(
            log=HarLog(
                version='1.2',
                creator=HarCreator(
