...
from ljp_page._modules.pydoll.protocol.emulation.types import UserAgentBrandVersion, UserAgentMetadata
...
class UserAgentParser:
    """无状态解析器从用户代理字符串中提取一致的元数据。

    给定一个 UA 字符串，例如：
        Mozilla/5.0（Windows NT 10.0；Win64；x64）AppleWebKit/537.36
        （KHTML，如 Gecko）Chrome/120.0.6099.109 Safari/537.36

    它生成 CDP Emulation.setUserAgentOverride 所需的所有元数据
    和 JavaScript 导航器属性覆盖，确保完全一致性
    HTTP 标头和 JS 属性之间。"""

    @staticmethod
    def parse(user_agent: str) -> ParsedUserAgent:
        """将用户代理字符串解析为一致的浏览器元数据。

        参数：
            user_agent：完整的用户代理字符串。

        返回：
            ParsedUserAgent 包含平台、供应商、appVersion、
            userAgentMetadata 和 JS 覆盖脚本。"""
