...
class RuntimeEvent(str, Enum):
    """来自 Chrome DevTools 协议运行时域的事件。

    此枚举包含与运行时相关的事件的名称，这些事件可以是
    从 Chrome DevTools 协议收到。这些事件提供了信息
    关于 JavaScript 执行、控制台 API 调用、异常和执行上下文。"""
...
class ExecutionContextCreatedEventParams(TypedDict):
    """executionContextCreated 事件的参数。"""

    context: ExecutionContextDescription


class ExecutionContextDestroyedEventParams(TypedDict):
    """executionContextDestroyed 事件的参数。"""
...
class ExecutionContextsClearedEventParams(TypedDict):
    """executionContextsCleared 事件的参数。"""

    pass


class InspectRequestedEventParams(TypedDict):
    """spectRequested 事件的参数。"""
