...
from ljp_page._core._exceptions import CloseFileException, OpenFileException


class _FileHandlerBase(Ljp_Logger):
    """异步文件句柄池基类。"""
    ...
    def _is_reusable_handle(self, file_obj: Any, mode: str, encoding: str) -> bool:
...
class _DirectoryBase(Ljp_Logger):
    """目录分配器基类。"""
...
class ManagedAsyncFile(Ljp_Logger):
    """可自行管理生命周期的异步文件对象。"""
