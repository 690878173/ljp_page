...
class CatBoostClassifierModel(BaseModel):
    ...
    def preprocess(self, scale: bool = False) -> "CatBoostClassifierModel":
        """特征预处理。"""
    ...
    def fit(self, scale: bool = False, **kwargs) -> "CatBoostClassifierModel":
        """训练 CatBoost 分类模型。"""
...
def catboost_classifier_auto(
    X: Union[np.ndarray, pd.DataFrame],
    y: Union[np.ndarray, pd.Series],
    scale: bool = False,
    **kwargs,
) -> tuple[CatBoostClassifierModel, np.ndarray]:
    """自动训练 CatBoost 分类并返回训练集预测结果。"""
...
__all__ = ["CatBoostClassifierModel", "catboost_classifier_auto"]
