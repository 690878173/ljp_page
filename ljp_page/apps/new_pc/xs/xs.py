...
from ljp_page._core._exceptions import MeetCheckError, No, Notfound, HtmlParseError
from ljp_page._modules.file import ManagedAsyncFile
from ljp_page._modules.request import LjpResponse

from ..base.model import P1Result
from ..base.pc import BasePc


class XsManager(Ljp_Logger):
    ...
    def __init__(
        ...
        self.expected_id = 1
        self.pending: dict[int, BasePc.P3Item] = {}
    ...
    async def add_p3(self, p3: BasePc.P3Item) -> None:
        try:
    ...
    @classmethod
    def _get_p_mode(cls, title: str, index: int,rt=False) -> str:
    ...
    def _clean_content(content: str) -> str:
        if not content:
            return ""
        lines = content.replace("\r", "").replace("\u3000", "  ").replace("\x00", "").replace('\u00A0','').replace('章节错误,点此报送(免注册)','').replace('报送后维护人员会在两分钟内校正章节内容,请耐心等待。','').strip().splitlines()
...
class Xs(BasePc):
    ...
    def parse_p2(self, res_html: str, url: str) -> BasePc.P2Result:
        raise NotImplementedError("需要继承实现 parse_p2")

    def parse_p3(self, res_html: str, url: str) -> BasePc.P3Item:
    ...
    async def _p1_work(self, p1_id: Any) -> list[Any]:
        ...
        page_url = self.config.p1_url.format(p1_id)

        response = await self.get(session=self.session, url=page_url)
        if not response:
            return []
    ...
    async def p2_work(self, p2_id):
        try:
            p2_result = await self._fetch_p2(p2_id)
    ...
    async def _fetch_p2(self, p2_items) -> BasePc.P2Result:
        if not self.config.p2_url:
            raise ValueError("config.p2_url 参数未设置")
        p2_id = p2_items.url
        base_url = self.format_p2_url(p2_id)
        current_url = base_url

        all_p3s: list[BasePc.P3Item] = []
        title = ""
        author = ""
        description = ""
    ...
    async def _parse_p3_info(
        ...
        p3.url = self.format_p3_url(p3.url)
        current_url = p3.url
    ...
    async def download(self, p2_result: BasePc.P2Result) -> None:
        ...
        try:
            for p2_item in p2_result.items:
                ...
                finished_path = self.directory.find_file_path(final_name)
                if finished_path is not None:
                    self.info(f"检测到完整文件，跳过下载: {finished_path}")
                    continue

                download_path = self.directory.find_file_path(download_name)
                if download_path is None:
                    download_path = self.directory.get_file_path(download_name)
                final_path = download_path.with_name(final_name)
                file_handle = ManagedAsyncFile(download_path, mode="a+", encoding="utf-8", logger=self.logger)
                ...
                async def _chapter_task(chapter_id: int, chapter: BasePc.P3Item) -> None:
            ...
            raise No("下载流程失败", e=exc)


    def _normalize_p3_items(self, raw_p3s: list[Any], p2_name: str) -> list[BasePc.P3Item]:
        p3_items: list[BasePc.P3Item] = []
    ...
    def _coerce_p3_item(
        ...
        item: Any,
        *,
        fallback: BasePc.P3Item | None,
        p2_name: str,
        p3_id: int,
    ) -> BasePc.P3Item:
        if isinstance(item, self.P3Item):
            if not item.id:
                item.id = p3_id
            if not item.p2_name:
        ...
        if isinstance(item, (tuple, list)) and len(item) == 2:
            return self.P3Item(url=str(item[1]), name=str(item[0] or ""), p2_name=p2_name, id=p3_id)
    ...
    @staticmethod
    def _reindex_p3_items(p3_items: list[BasePc.P3Item], p2_name: str) -> None:
