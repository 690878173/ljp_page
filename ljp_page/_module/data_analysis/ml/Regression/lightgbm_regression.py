...
class LightGBMRegressionModel(BaseModel):
    ...
    def preprocess(self, scale: bool = False) -> "LightGBMRegressionModel":
        """特征预处理。"""
    ...
    def fit(self, scale: bool = False, **kwargs) -> "LightGBMRegressionModel":
        """训练 LightGBM 回归模型。"""
...
def lightgbm_regression_auto(
    X: Union[np.ndarray, pd.DataFrame],
    y: Union[np.ndarray, pd.Series],
    scale: bool = False,
    **kwargs,
) -> tuple[LightGBMRegressionModel, np.ndarray]:
    """自动训练 LightGBM 回归并返回训练集预测值。"""
    model = LightGBMRegressionModel(X, y)
    model.fit(scale=scale, **kwargs)
    predictions = model.predict(X)
    return model, predictions


__all__ = ["LightGBMRegressionModel", "lightgbm_regression_auto"]
