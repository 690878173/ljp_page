...
class RandomForestClassifierModel(BaseModel):
    ...
    def preprocess(self, scale: bool = False) -> "RandomForestClassifierModel":
        """
        特征预处理。

        说明：树模型不依赖标准化，默认 `scale=False`。
        """
    ...
    def fit(self, scale: bool = False, **kwargs) -> "RandomForestClassifierModel":
        """训练随机森林分类器。"""
...
def random_forest_classifier_auto(
    X: Union[np.ndarray, pd.DataFrame],
    y: Union[np.ndarray, pd.Series],
    scale: bool = False,
    **kwargs,
) -> tuple[RandomForestClassifierModel, np.ndarray]:
    """自动训练随机森林分类并返回训练集预测结果。"""
    model = RandomForestClassifierModel(X, y)
    model.fit(scale=scale, **kwargs)
    predictions = model.predict(X)
    return model, predictions


__all__ = ["RandomForestClassifierModel", "random_forest_classifier_auto"]
