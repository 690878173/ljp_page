...
class YhdmSpider(Ys):
    ...
    def parse_video_info(html_str: str, video_id: Any, url: str) -> VideoInfo:
        ...
        for index, node in enumerate(episode_nodes, start=1):
            ...
            episode_title = (
                YhdmSpider._clean_text("".join(node.xpath(".//text()"))) or f"第{index}集"
            )
            episodes.append(
                VideoEpisode(
                    index=index,
    ...
    async def get_real_m3u8_url(self, episode_page_url: str) -> str | None:
        response = await self.req.get(self.session, episode_page_url)
        match = self.M3U8_RE.search(response.text or "")
