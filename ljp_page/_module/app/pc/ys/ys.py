...
class Ys(BasePc):
    ...
    async def _p1_work(self, p1_id: Any) -> list[Any]:
        ...
        response = await self.req.get(self.session, page_url)
        if not response:
            return []
    ...
    async def _fetch_m3u8_text(self, url: str) -> str:
        response = await self.req.get(self.session, url)
        text = response.text
        if not text:
    ...
    async def _download_one_segment(self, index: int, segment_url: str, paths) -> bool:
        ...
        for retry_index in range(self.config.segment_retry + 1):
            try:
                response = await self.req.get(self.session, segment_url, check_fp=False)
    ...
    async def _fetch_video_info(self, p2_id: Any) -> VideoInfo:
        ...
        response = await self.req.get(self.session, detail_url)
    ...
    def _format_p2_source_url(self, source: Any) -> str:
        ...
        return self.config.format_p2_url(raw_url)
    ...
    def _coerce_episode(self, item: Any, index: int) -> VideoEpisode:
        ...
        if isinstance(item, P3Item):
            return VideoEpisode(
                ...
                page_url=self.config.format_p3_url(item.url),
        ...
        if isinstance(item, dict):
            ...
            return VideoEpisode(
                ...
                page_url=self.config.format_p3_url(page_url),
        ...
        if isinstance(item, (tuple, list)) and len(item) >= 2:
            return VideoEpisode(
                ...
                page_url=self.config.format_p3_url(item[1]),
