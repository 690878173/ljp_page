...
class M3u8Parser(Ljp_BaseClass):
    """支持 master/media 两类 m3u8，并自动下钻到媒体流。"""
    ...
    async def resolve(
        ...
        *,
        ...
        参数:
            url: 初始 m3u8 地址。
            fetch_text: 拉取 m3u8 文本的异步函数。
            max_depth: master playlist 最大下钻层数。
        返回值:
            M3u8Playlist: 最终媒体播放列表。
        """
        ...
        for _ in range(max_depth + 1):
            ...
            if not segments:
                raise ValueError(f"m3u8 未解析到分片: {current_url}")
            return M3u8Playlist(url=current_url, segment_urls=segments)

        raise RecursionError(f"m3u8 解析层级超过限制: {url}")
