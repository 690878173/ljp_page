...
class VideoManager(BaseVideoManager):
    ...
    async def download_video(self, session: Any, video_info: VideoInfo) -> None:
        ...
        results = await self.spider.exc.gather_inside(tasks, return_exceptions=True)
        for item in results:
    ...
    async def _download_episode(
        ...
        if self.config.cleanup_segments:
            await self.spider.exc.run_in_thread(self.storage.cleanup_segment_dir, paths.segment_dir)
    ...
    async def _download_one_segment(
        ...
        for retry_index in range(self.config.segment_retry + 1):
            try:
                ...
                await self.spider.exc.run_in_thread(target_file.write_bytes, content)
                return True
