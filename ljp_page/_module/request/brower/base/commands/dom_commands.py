...
from ..protocol.dom.methods import (
...
if TYPE_CHECKING:
    ...
    from ..protocol.dom.types import (
        IncludeWhitespace,
        LogicalAxes,
        PhysicalAxes,
        RelationType,
...
class DomCommands:
    ...
    def describe_node(
        ...
        """
        获取指定 DOM 节点的详细信息（无需先启用 DOM 域）。
        适用于快速检查单个元素，无需监听 DOM 变更事件。

        参数：
            node_id: 客户端已知的节点 ID。
            backend_node_id: 浏览器内部使用的后端节点 ID。
            object_id: 节点包装对象的 JavaScript 对象 ID。
            depth: 获取子节点的最大深度，默认 1；-1 表示全子树。
            pierce: 是否穿透 iframe 和 Shadow DOM（默认 False）。
    ...
    def disable() -> DisableCommand:
        """禁用当前页面的 DOM 代理。停止 DOM 事件，阻止后续 DOM 操作，直到重新启用。"""
        return Command(method=DomMethod.DISABLE)

    @staticmethod
    def enable(include_whitespace: Optional['IncludeWhitespace'] = None) -> EnableCommand:
        """
        启用当前页面的 DOM 代理（大部分 DOM 操作的前置条件）。
        启用后会开始接收 DOM 结构变化等事件。

        参数：
            include_whitespace: 是否在返回的节点子数组中包含纯空白文本节点。
                                 取值："none" 或 "all"。
    ...
    def focus(
        ...
        参数：
            node_id / backend_node_id / object_id: 三种方式指定目标节点。
        返回：聚焦命令。
        """
        params = FocusParams()
        if node_id:
    ...
    def get_attributes(node_id: int) -> GetAttributesCommand:
        """
        R获取指定节点的所有属性名和属性值。
        返回交错数组：[name1, value1, name2, value2, ...]
    ...
    def get_box_model(
        ...
        """
        获取节点的盒模型信息（内容、内边距、边框、外边距的坐标）。
        用于精确定位元素在页面中的位置和尺寸。

        参数：node_id / backend_node_id / object_id 三选一。
        返回：包含盒模型数据的命令。
        """
